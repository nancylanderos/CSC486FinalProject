...
#price and event type scatterplot
# x = data['price']
# y = data['event_type']
# plt.xlabel("price")
# plt.ylabel("event_type")
...
# plt.axis('off')
# plt.show()
#brands customers navigate through
topcateg = 30
topcateg1 = data['brand'].value_counts()[:topcateg].sort_values(ascending=False)
df = pd.DataFrame({'count':topcateg1, "topcateg1":topcateg1})
squarify.plot(sizes=topcateg1, label=topcateg1.index.array, color=["red","blue","green","grey","cyan"], alpha=.7)
plt.axis('off')
plt.show()
...
# print (viewed1)
#what items are placed in a cart?
# cart = data.loc[data['event_type'] == "cart"]
# cart1 = cart.dropna(axis = 'rows')
# print (cart1)
